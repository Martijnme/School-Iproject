            ...
            <div class="container p-0">
                <div class="row justify-content-center">
                    <section class="row pt-5">
                                ...
                                <div class="col-12 col-sm-6 col-md-6 col-lg-3 col-xl-3 col-xxl-3">
                                    <div class="col-12 card">
                                        <div class="card-body" style="text-align: center">
                                            <a class="card-title m-0" style="text-decoration: none; color: black;" href="category.php?rubriek=<?= $item['rubrieknaam'] ?>"> <?= $item['rubrieknaam']  ?></a>
                                        </div>
                                    </div>
                                </div>
                        <?php }
                        ...
                        <section class="row">
                            <?php
                            if($_GET['rubriek'] != 'Root'){
                                echo "<h2>Veilingen</h2>";
                            }
                            if (empty($categoryVeilingen) && $_GET['rubriek'] != 'Root') { ?>
                                <p style="color: red;">Sorry! Deze categorie heeft geen veilingen</p>
                                <?php if (isset($_SESSION['loggedIn']) && $_SESSION['loggedIn']) { ?>
                                    <a href="../pages/veilingplaatsen.php" class="btn btn-primary d-grid">Nieuwe veiling aanbieden</a>
                                <?php }
                            } else {
                                foreach ($categoryVeilingen as $auctionItem) { ?>
                                    <div class="col-12 col-sm-6 col-md-6 col-lg-3 col-xl-3 col-xxl-3">
                                        <div class="col-12 card">
                                        <?php $images = $auction->getImages($auctionItem['voorwerp']);
                                        if($images != NULL) {?>
                                            <img class="card-img-top" src="../../pics/<?= $images['filenaam']?>" alt="<?=images['filenaam']?>">
                                        <?php }else {?>
                                             <img class="card-img-top" src="../../images/placeholder1.jpg" alt="Placeholder"> 
                                        <?php } ?>
                                            <div class="card-body">
                                                <h5 class="card-title" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis; width: 150px;"><?= $auctionItem['titel'] ?></h5>
                                                <p class="veilingStatusOpen"><?=strtoUpper($auctionItem['veilingGesloten'])?></p>
                                            <div class="card-text d-flex justify-content-between beschrijving">
                                                <p class="huidigeBod">Huidig bod:</p>
                                                <p class="prijs">€<?php if(isset($auctionItem['bod'])){echo $auctionItem['bod'];}else{ echo $auctionItem['startprijs'];}?></p>
                                            </div>
                                                <a href="../../src/pages/veiling.php?veilingID=<?=$auctionItem['voorwerp']?>" class="btn btn-primary d-grid">BEKIJKEN</a>
                                            </div>
                                        </div>
                                    </div>
                            <?php }
